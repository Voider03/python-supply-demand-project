...
class Buyer:
    ...
    def buy(self):
        if self.shopsVisited:
            ...
            chosen_seller.earned += self.boughtPrice
...
class Seller:
    def __init__(self, Id, mp, ep, stock, dp, bp, stp):
        self.Id = Id
    ...
    def cleanup(self):
        if self.daily_stock < (self.stock * self.stp):
            if self.earned > (self.stock - self.daily_stock) * self.mp:
                self.earned -= ((self.stock - self.daily_stock) * self.mp)
                self.daily_stock = self.stock
...
def detSel(numSellers):
    for i in range(numSellers):
        ...
        dp = float(input(f"Enter Buyer {i} discount percentage: "))
        bp = float(input(f"Enter Buyer {i} maximum discount percentage: "))
        stp = float(input(f"Enter Buyer {i} manimum stock percentage: "))

        sellers.append(Seller(i, mp, ep, stock, dp, bp, stp))
